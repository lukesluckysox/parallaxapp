...
interface AuthContextType {
  ...
  register: (username: string, password: string, displayName?: string, demographics?: { age: string; gender: string; location: string }) => Promise<{ ok: boolean; error?: string }>;
  logout: () => void;
  ...
  const register = useCallback(async (username: string, password: string, displayName?: string, demographics?: { age: string; gender: string; location: string }) => {
    try {
      const res = await fetch("./api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username, password, displayName, ...demographics }),
        credentials: "same-origin",
      });
      if (!res.ok) {
